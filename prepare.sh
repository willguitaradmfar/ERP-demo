...
APP_LOG_PATH="${APP_LOG_PATH:-/tmp/app.log}"
...
start_services() {
  ...
  for SVC in "${SERVICES[@]}"; do
    ...
    # Etapa 2: Iniciar serviço em background (logs redirecionados para APP_LOG_PATH)
    log_info "[$name] Executando: $cmd"
    log_info "[$name] Logs sendo redirecionados para: $APP_LOG_PATH"

    # Executa comando com logs prefixados pelo nome do serviço
    bash -c "exec $cmd 2>&1 | while IFS= read -r line; do echo \"[$name] \$line\"; done >> \"$APP_LOG_PATH\"" &
    local pid=$!
