...
export function ReceivablesList({ initialReceivables, customers, categories }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingItem
        ? `/api/receivables/${editingItem.id}`
        : '/api/receivables'
  ...
  const handlePay = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/receivables/${payingItem.id}/pay`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        title={editingItem ? 'Editar Recebível' : 'Novo Recebível'}
        size="lg"
      >
      ...
      >
        <form onSubmit={handlePay} className="space-y-4">
          <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
            <p className="text-sm text-gray-500 dark:text-gray-400">Recebível</p>
