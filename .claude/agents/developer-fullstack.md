...
**Actions:**
1. Use the MCP tool `mcp__kanban-api__search_knowledge_base` to search technical rules and patterns
2. ALWAYS consult these queries based on your Step 1 analysis:

   **For Backend:**
   ```javascript
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "REST API patterns", limit: 5 })
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "backend folder structure", limit: 5 })
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "how to create use-case in backend", limit: 5 })
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "DTO validation", limit: 5 })
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "database integration", limit: 5 })
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "backend unit tests", limit: 5 })
   ```
   ...
   **For specific functionalities (if applicable):**
   ```javascript
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "authentication and authorization", limit: 5 })
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "file upload", limit: 5 })
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "pagination and filters", limit: 5 })
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "cache with Redis", limit: 5 })
   mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "jobs and queues with Bull", limit: 5 })
   ```

3. **Important:** Base ALL your implementation on the found rules and patterns

**Tip:** Semantic search returns results based on query meaning, being more effective than searching for specific keywords.
...
4. **Use Cases** - Business rules (search "how to create use-case" in knowledge base)
...
## Knowledge Base (kanban-api)

```javascript
// Search project knowledge base for architecture patterns and best practices
mcp__kanban-api__search_knowledge_base({
  projectId: <from_metadata>,
