...
## STEP 4: CONSULT TECHNICAL RULES IN KNOWLEDGE BASE

**Objective:** Search the project knowledge base for architecture patterns, development standards, and best practices mentioned in the task.

**Actions:**
1. Re-read task file and identify mentions to technical rules (e.g., "follow use-case pattern", "use DTO validation")
2. Use `mcp__kanban-api__search_knowledge_base` to search technical rules:

**Query examples:**
```javascript
mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "REST API patterns", limit: 5 })
mcp__kanban-api__search_knowledge_base({ projectId: <from_metadata>, query: "how to create use-case", limit: 5 })
